...
const WORKFLOW_TYPES = ['release-snapshot', 'release', 'test', 'sonar-qube'];
const TYPE_ORDER = { release: 0, 'release-snapshot': 1, test: 2, 'sonar-qube': 3 };
...
function parseFilename(filename) {
  ...
  for (const type of WORKFLOW_TYPES) {
    ...
    if (base.endsWith(suffix)) {
      ...
      return { kind: 'app', filename, segments, type };
    }
  }
